  ...
  return (
    <div ref={ref} className="relative">
      <input
        ...
        className="w-48 lg:w-64 px-3 py-1.5 rounded-lg border border-forest-700/40 bg-forest-900/50 text-white placeholder-forest-500 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
        value={query}
        onChange={e => { setQuery(e.target.value); setOpen(true); }}
        onFocus={() => results.length > 0 && setOpen(true)}
      />
      {open && results.length > 0 && (
        <div className="absolute top-full mt-1 w-72 bg-forest-800 rounded-lg shadow-xl border border-forest-700/40 z-50 max-h-80 overflow-y-auto">
          {results.map(a => (
            <button
              key={a.id}
              className="flex items-center gap-3 w-full px-3 py-2 hover:bg-forest-700/50 text-left"
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-white">{a.common_name}</div>
